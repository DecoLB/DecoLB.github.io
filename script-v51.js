...
document.addEventListener('mousemove', (e) => {
  __trailTick++;
  if (__trailTick % 2 !== 0) return;
  const d = document.createElement('i');
  d.className = 'mouse-trail-dot';
...
document.querySelectorAll('.cycle-counter').forEach((card, idx) => {
  ...
  async function run() {
    while (true) {
      await __tween(1400, t => {
        valueEl.textContent = prefix + Math.round(target * t) + suffix;
      });
      await __sleep(2000);
      await __tween(500, t => {
        valueEl.textContent = prefix + Math.round(target * (1 - t)) + suffix;
      });
      await __sleep(350);
    }
  }
  setTimeout(run, idx * 170);
});
