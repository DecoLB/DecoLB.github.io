...
\n// V5 cursor trail\nlet __trailTick=0;document.addEventListener('mousemove',e=>{__trailTick++;if(__trailTick%2)return;const d=document.createElement('i');d.className='mouse-trail-dot';d.style.left=e.clientX+'px';d.style.top=e.clientY+'px';document.body.appendChild(d);setTimeout(()=>d.remove(),760)},{passive:true});\n// V5 scale counters\nfunction __sleep(ms){return new Promise(r=>setTimeout(r,ms))}function __tween(duration,cb){return new Promise(res=>{const st=performance.now();function fr(now){const t=Math.min(1,(now-st)/duration),e=1-Math.pow(1-t,4);cb(e);if(t<1)requestAnimationFrame(fr);else res()}requestAnimationFrame(fr)})}\ndocument.querySelectorAll('.cycle-counter').forEach((card,idx)=>{const s=card.querySelector('strong'),target=Number(card.dataset.target||0),prefix=card.dataset.prefix||'',suffix=card.dataset.suffix||'';const run=async()=>{while(true){await __tween(1400,t=>s.textContent=prefix+Math.round(target*t)+suffix);await __sleep(2000);await __tween(500,t=>s.textContent=prefix+Math.round(target*(1-t))+suffix);await __sleep(350)}};setTimeout(run,idx*170)});\n